...
export default class Lark extends Notify {
  ...
  async uploadLocalFile(url: string, type: 'path' | 'base64'): Promise<string> {
    ...
    if (!(LARK_APP_ID && LARK_APP_SECRET)) {
      core.setFailed(
        `Action failed with error missing onf of [LARK_APP_ID, LARK_APP_SECRET]`
      )

      return ''
    }
    const tenant_access_token = await this.getAccessToken(
      LARK_APP_ID,
      LARK_APP_SECRET
    )
    ...
    const uploadRes = await axios.request(request_config).catch(err => {
      core.debug(
        `img upload fail code: ${err.code} body: ${JSON.stringify(err.response.data)}`
      )
      return err.response
    })
  ...
  genSin(
    signKey: string | undefined = this.signKey,
    timestamp: string
  ): string {
    const crytoStr = `${timestamp}\n${signKey}`
    const signature = CryptoJs.enc.Base64.stringify(
      CryptoJs.HmacSHA256('', crytoStr)
    )
